...
// faker is a JS library which can create fake data for us
...
export const UberProvider = ({ children }) => {
  ...
  // for connecting with the metamask->
  let metamask
  ...
  // check if user exists and if exists return the data 
  useEffect(() => {
...
// end of wallet connection
  ...
  // for sending request to the database to log details of user if it doesn't exists already
    ...
    try {
      await fetch('/api/db/createUser', {
        ...
        body: JSON.stringify({
          ...
          // faker will create randomly fake name when we will create user ->
